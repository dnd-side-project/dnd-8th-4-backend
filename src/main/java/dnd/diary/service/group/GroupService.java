...
public class GroupService {
    ...
    @Transactional
    public GroupCreateResponse createGroup(Long userId, MultipartFile multipartFile, String groupName, String groupNote) {
        User hostUser = userService.getUser(userId);
    ...
    @Transactional
    public GroupUpdateResponse updateGroup(Long userId, MultipartFile multipartFile, Long groupId, String groupName, String groupNote) {
...
//		groupRepository.delete(group);
    ...
    @Transactional
    public GroupStarResponse starGroup(Long groupId, Long userId) {
        User user = userService.getUser(userId);
        Group group = findGroup(groupId);
    ...
    // 내가 속한 그룹 목록 조회
    public GroupListResponse getGroupList(Long userId) {
        User user = userService.getUser(userId);
    ...
    public List<GroupSimpleResponse> getGroupSimpleList(Long userId) {
        User user = userService.getUser(userId);
        List<UserJoinGroup> userJoinGroupList = user.getUserJoinGroups();
    ...
    public List<GroupStarListResponse> getGroupStarList(Long userId) {
        User user = userService.getUser(userId);
    ...
    public GroupListResponse searchGroupList(String keyword, Long userId) {
        User user = userService.getUser(userId);
    ...
    // 그룹 초대
    @Transactional
    public GroupInviteResponse inviteGroupMember(GroupInviteRequest request, Long userId) {
        ...
        for (Long userIds : request.getInvitedUserIdList()) {
            User invitedUser = userRepository.findById(userIds).orElseThrow(() -> new CustomException(NOT_FOUND_USER));
